...
abstract class ConfigurationClassUtils {
	...
	public static boolean checkConfigurationClassCandidate(BeanDefinition beanDef, MetadataReaderFactory metadataReaderFactory) {
		...
		if (beanDef instanceof AnnotatedBeanDefinition &&
			...
			// ((AnnotatedBeanDefinition) beanDef).getMetadata() = StandardAnnotationMetadata
			metadata = ((AnnotatedBeanDefinition) beanDef).getMetadata();
		}
		// 如果这个BeanDefinition不是AnnotationBeanDefinition，但是一个AbstractBeanDefinition则走这里
		...
		// 判断当前BeanDefinition类是否加了@Configuration注解，如果加了，设置configurationClass属性为full
		...
		// candidateIndicators.add(Import.class.getName());
		// candidateIndicators.add(ImportResource.class.getName());
		// 如果加了@Configuration注解，则不解析上面的四个注解，否则就解析上面这四种注解标注的类或者有@Bean方法的类
